...
- [`auth/email/[[path]].ts`](auth/email/%5B%5Bpath%5D%5D.ts) — the four `emailCodeAuth` handlers (`start`, `code`, `verify`, `poll`) at `/auth/email/*`, dormant until `RESEND_API_KEY` + `MAIL_FROM` exist. Provision those with [`scripts/provision-resend.mjs`](../../scripts/provision-resend.mjs); apply `migrations/0009_pending_auth.sql` + `0010_pending_auth_ip.sql` (or `schema.sql` on a fresh DB) first.
