...
- **Google.** Without `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET`, `/auth/google/*` answers 503, One Tap doesn't render, and the page says so rather than hiding the buttons. Google exposes no API to create the client; `scripts/provision-oauth-client.mjs` walks the one manual step.
