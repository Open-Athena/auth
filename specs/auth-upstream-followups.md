...
- ***Superseded (2026-09-25): one `name`, no pair at all.*** Migration 0013 folds `first`/`last` into `Subject.name` (grants and requests) and `profiles.name`; `askName` is a boolean, `ProfilePanel` has one Name field, and OIDC seeding takes the IdP's `name` claim whole. Splitting stayed lossy even as an opt-in.
