...
*Built: `POST <base>/track` on `authRoutes` (a client-supplied path is untrusted input landing in an admin's table, so same-origin paths only, capped at 512 chars; anonymous beacons are dropped silently). `core/bots.ts` filters `view` rows only — a lifecycle event from a bot is exactly what you want to see — using a deliberately conservative UA regex plus CF's own `CF-Verified-Bot`; a **missing** UA counts as automation. `rollupAccessLog()` + `0004_access_log_daily.sql` collapse raw rows older than N days into `(day, event, grant_id, path, country)` buckets and delete them; it's idempotent and accumulates into existing buckets, and the grouping keys are `COALESCE`d because SQLite treats NULLs as distinct in a PRIMARY KEY (without that, every NULL-path row gets its own bucket and never merges). Pages Functions have no scheduler, so a consumer drives the rollup from a Worker cron or an admin route.*

*Still not built: viewport/time-on-page signals (only route changes are beaconed), and per-request logging of **anonymous** traffic — today the log starts at the point someone has an identity, so the "how much of last week was Bob vs. strangers" question is still half-answered.*
