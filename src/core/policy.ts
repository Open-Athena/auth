...
export function domainPolicy(domains: readonly string[], scopes: readonly string[]): EmailPolicy {
...
export function anyEmailPolicy(scopes: readonly string[]): EmailPolicy {
  return () => [...scopes]
}

/** First policy to return non-null wins; scopes are not merged. */
export function firstMatch(...policies: EmailPolicy[]): EmailPolicy {
  return async email => {
