 ...
 */
import type { DecisionVerb, DecisionView } from './decisions.js'
import { EMAIL_LINK_ACTOR } from './decisions.js'
import { type AccessRequest, subjectName } from './requests.js'

const esc = (s: string): string =>
  s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')

const when = (ts: number | null): string => (ts === null ? 'an unknown time' : new Date(ts * 1000).toUTCString())

/** Never prints a name we don't have. See `EMAIL_LINK_ACTOR`. */
const by = (actor: string | null): string =>
  actor === null || actor === EMAIL_LINK_ACTOR ? 'via email link' : `by ${esc(actor)}`

const who = (r: AccessRequest): string => {
  const name = subjectName(r.subject) ?? r.name
  return esc(name ? `${name} <${r.email}>` : r.email)
}

const VERB_LABEL: Record<DecisionVerb, string> = { approve: 'Approve', deny: 'Deny' }
...
export function renderDecisionPage(view: DecisionView, opts: DecisionPageOptions = {}): Response {
  const { appName = 'this site', action = '' } = opts
  const page = (title: string, inner: string, status = 200): Response =>
    new Response(shell(title, inner), {
      status,
  ...
  switch (view.kind) {
    case 'invalid':
      ...
      return page(
        ...
        `<h1>This link is no longer valid</h1>
         <p>It may have expired, or the request may have been removed. Open ${esc(appName)} to review pending requests.</p>`,
        404,
      )
    ...
    case 'unauthorized':
      return page(
        'Sign in to decide',
        `<h1>Sign in to decide</h1>
         <p>${esc(appName)} requires an administrator session before a request can be decided.</p>`,
        401,
      )
    ...
    case 'confirm': {
      ...
      return page(
        `${VERB_LABEL[verb]} access request`,
        `<h1>${VERB_LABEL[verb]} this request?</h1>
         <dl>
        ...
        `<h1>${done}</h1>
         <p>${who(request)} ${
           verb === 'approve'
             ...
             : 'was not granted access, and has not been told.'
         }</p>
