...
/** The response every `start` returns, allowed address or not — no allowlist oracle. */
interface StartResult {
  status: 'sent'
  /** Names the pending flow for code entry and polling. Not a secret on its own. */
  id: string
}
...
const hashCode = (email: string, code: string): Promise<string> => hashToken(`${email}:${code}`)

/** Only same-origin paths, so a link's `?next=` can't become an open redirect. */
const safeNext = (raw: string | null, fallback: string): string =>
  raw && raw.startsWith('/') && !raw.startsWith('//') ? raw : fallback

/** A live row, or null: covers absent, already-consumed, and expired identically. */
function usable(row: PendingAuth | null, nowS: number): PendingAuth | null {
  if (!row || row.consumedAt !== null || row.expiresAt <= nowS) return null
  return row
}

export function emailCodeAuth(opts: EmailCodeOptions) {
  const {
    gate,
    store,
    send,
  ...
  async function start({
    ...
    // answer differently.
    if (!EMAIL_RE.test(email)) return json({ status: 'invalid-email' }, 400)
   ...
   */
  async function verifyLink({ request }: { request: Request }): Promise<Response> {
    ...
    const fail = (): Response =>
      new Response(null, { status: 302, headers: { location: next, 'cache-control': 'no-store' } })
   ...
   */
  async function verifyCode({ request }: { request: Request }): Promise<Response> {
    const body = (await request.json().catch(() => ({}))) as { id?: unknown; code?: unknown }
    ...
    if (!row || row.attempts >= maxAttempts) return invalid
    ...
    const signedIn = await gate.signIn(row.email, request)
    if (!signedIn) return json({ ok: false }, 403)
   ...
   */
  async function poll({ request }: { request: Request }): Promise<Response> {
