...
export function bytesToDataUri(type: string, bytes: Uint8Array): string {
  let binary = ''
...
export function validateUploadedImage(
  bytes: Uint8Array,
  { maxBytes = MAX_INLINE_AVATAR_BYTES }: { maxBytes?: number } = {},
): ValidatedImage {
  if (!bytes.length) throw new InvalidImageError('empty upload')
  if (bytes.length > maxBytes) throw new InvalidImageError(`image is ${bytes.length} bytes, over the ${maxBytes} cap`)
  const sniffed = sniffImage(bytes)
  if (!sniffed) throw new InvalidImageError('not a supported image (png, jpeg, webp, or gif)')
...
function jpegDimensions(bytes: Uint8Array, view: DataView): { width: number; height: number } | null {
  let offset = 2
...
function webpDimensions(bytes: Uint8Array, view: DataView): { width: number; height: number } | null {
