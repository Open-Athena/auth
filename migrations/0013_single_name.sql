-- One name, not a given/family pair. Name structure and order vary too much
-- across cultures ("given" first or last, one name, several family names) for
-- a split to be anything but lossy, so a person is `name` everywhere: the
-- `profiles` column, and `name` inside `grants.subject_json` /
-- `access_requests.subject_json`. Existing pairs are joined "first last".
